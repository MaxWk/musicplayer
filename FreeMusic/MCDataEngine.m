...
//        NSLog(@"resp:%@",respJson);
        @try{
            ...
            if ([[dictionary allKeys] count]>1) {
                ...
                BOOL  sucess = [WKSingerModel insertToDB:model];
                if (sucess) {
                    DEBUGLog(@"%@ sucess %lld",model.name,model.ting_uid);
                }
                
            }else{
                int errorcode = [[dictionary objectForKey:error_code] intValue];
                DEBUGLog(@"%d",errorcode);
            }
            completion(array);
        }
...
//        NSLog(@"resp:%@",respJson);
        WKSongList * list = [WKSongList new];
        @try{
            NSDictionary * dictionary = respJson;
            if ([[dictionary allKeys] count]>1) {
...
//        NSLog(@"resp:%@",respJson);
        WKSongModel * song = [WKSongModel new];
        @try{
            NSDictionary * dictionary = respJson;
            if ([[dictionary allKeys] count]>1) {
                ...
                for (NSDictionary * sub in songList) {
                    ...
                    song.pic_big = [sub objectForKey:@"songPicBig"];
                    song.file_duration = [[sub objectForKey:@"time"] intValue];
                }
            }else{
                int errorcode = [[dictionary objectForKey:error_code] intValue];
                DEBUGLog(@"%d",errorcode);
...
{
    self.showError = YES;
    KHNetworkEngine * netwrok = [self initWithHostName:@"musicdata.baidu.com"];//26
    NSRange range;
    range.length = 27;
    range.location = 0;
    NSString *nowlrclink = [lrclink stringByReplacingCharactersInRange:range withString:@""];
    MKNetworkOperation *op = [netwrok operationWithPath:nowlrclink];
        [op addCompletionHandler:^(MKNetworkOperation *completedOperation) {
