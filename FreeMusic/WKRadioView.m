...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationDidEnterBackgroundControlRadioStatus:) name:WKRFRadioViewStatusNotifiation object:nil];
        
        audioPlayer = [[AudioPlayer alloc] init];
        audioPlayer.delegate = self;

        _currentPlaybackTime = [[UILabel alloc] initWithFrame:CGRectMake(30, 5, 50, 25)];
        ...
        lrcView = [[WKLrcView alloc] initWithFrame:CGRectMake(30, progress.size.height+progress.origin.y+10, self.size.width, self.size.height-100)];
        [self addSubview:lrcView];
        
        backImageView = [[WKPAImageView alloc] initWithFrame:CGRectMake(frame.size.width/2, frame.size.height-64-10, 64, 64)];
        [self addSubview:backImageView];
        
        _playButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _playButton.frame = CGRectMake(frame.size.width/2, frame.size.height-64-10, 64, 64);
        ...
        _playbackButton.frame =  CGRectMake(30, _preButton.origin.y, 48, 48);
        ...
        _playListButton.frame =  CGRectMake(self.size.width-23, _preButton.origin.y, 48, 48);
        ...
        [_collectButton setImage:[UIImage imageNamed:@"collected"] forState:UIControlStateSelected];
...
{
    [backImageView setImageWithURL:[NSURL URLWithString:_songModel.pic_big]];
...
- (void)setSelectedPlaylistItem:(FSPlaylistItem *)selectedPlaylistItem  {
    if (_selectedPlaylistItem != selectedPlaylistItem) {
        ...
        _totalPlaybackTime.text = [self TimeformatFromSeconds:_songModel.file_duration];
...
- (void)collectButtonEvent:(UIButton *)button
{
    if (button.selected) {
        button.selected = NO;
    }else{
